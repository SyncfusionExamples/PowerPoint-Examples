    ...
    internal class Program
    {
        static void Main()
        {
            ...
            using (IPresentation pptxDoc = Presentation.Open(@"../../Data/Template.pptx"))
			{
                ...
                using (Stream stream = pptxDoc.Slides[0].ConvertToImage(Syncfusion.Drawing.ImageFormat.Emf))
                {
                    //Create a bitmap of specific width and height.
                    using (Bitmap bitmap = new Bitmap(customWidth, customHeight, PixelFormat.Format32bppPArgb))
                    {
